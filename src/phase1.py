...
def inverted_index_construction(data, normalizer, tokenizer, lemmetizer, stopwords):
    tokens_dict = dict()
...
def response_to_query(*, inverted_index, query, data, normalizer, tokenizer, lemmetizer, stopwords):
    # Query processing
    ...
    query = [token for token in query if token in inverted_index.keys(
    ...
    if and_tokens:
        for i in range(len(and_tokens)):
            if i == 0:
                intersection = set(inverted_index[and_tokens[i]].keys())
                union = intersection
                continue

            intersection = intersection.intersection(
                set(inverted_index[and_tokens[i]].keys()))
            union = union.union(
                set(inverted_index[and_tokens[i]].keys()))
        ...
        for i in range(len(not_tokens)):
            if not intersection:
                intersection = refrence - \
                    set(inverted_index[not_tokens[i]].keys())
                if not union:
                    union = intersection
                    continue
            ...
            intersection = intersection.intersection(
                refrence - set(inverted_index[not_tokens[i]].keys()))
            union = union.union(
                refrence - set(inverted_index[not_tokens[i]].keys()))
    ...
    if and_phrase:
        for phrase in and_phrase:
            ...
            for i in range(len(phrase)):
                if i == 0:
                    phrase_intersection = set(
                        inverted_index[phrase[i]].keys())
                    continue

                phrase_intersection = phrase_intersection.intersection(
                    set(inverted_index[phrase[i]].keys()))
            ...
            for id in tmp_phrase_intersection:
                tmp = inverted_index[phrase[0]][id]["positions"]
                for j in range(1, len(phrase)):
                    tmp = [x for x in tmp if x +
                        j in inverted_index[phrase[j]][id]["positions"]]
...
def main():
    ...
    inverted_index = inverted_index_construction(data, normalizer, tokenizer, lemmetizer, stopwords)
    query = input("Enter your query : ")
    response_to_query(inverted_index=inverted_index, query=query, data=data, normalizer=normalizer,
                       tokenizer=tokenizer, lemmetizer=lemmetizer, stopwords=stopwords)
